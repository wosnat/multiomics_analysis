...
This section owns the *methodology* — which background to use and why.
For the *computation* — the enrichment building blocks the explorer
ships (`de_enrichment_inputs`, `cluster_enrichment_inputs`, `fisher_ora`,
`signed_enrichment_score`, `EnrichmentResult`) and worked recipes — read
`docs://analysis/enrichment`. Use those rather than reimplementing the
Fisher math by hand.
